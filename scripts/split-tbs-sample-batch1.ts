/**
 * Split TBS_SAMPLE_FOR_REVIEW.md (Batch 1) into smaller chunks
 * for ChatGPT review (to avoid token limits)
 */

import * as fs from "fs";

const content = fs.readFileSync("./TBS_SAMPLE_FOR_REVIEW.md", "utf-8");
...
console.log(`Found ${tbsBlocks.length} TBS blocks in Batch 1`);

// Split into chunks of 15 TBS each
const CHUNK_SIZE = 15;
const chunks: string[][] = [];
...
for (let i = 0; i < chunks.length; i++) {
  const chunkHeader = `# TBS Sample for Review - Batch 1, Part ${i + 1} of ${chunks.length}
...
`;
  ...
  const filename = `./TBS_SAMPLE_BATCH1_PART${i + 1}.md`;
...
console.log("\nDone! Files created:");
for (let i = 0; i < chunks.length; i++) {
  console.log(`  - TBS_SAMPLE_BATCH1_PART${i + 1}.md`);
}
