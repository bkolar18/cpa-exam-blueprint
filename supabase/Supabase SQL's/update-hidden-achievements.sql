...
SELECT
  ...
  ROUND(100.0 * SUM(CASE WHEN is_hidden THEN 1 ELSE 0 END) / COUNT(*), 1) AS hidden_percentage
FROM achievements;

-- Show all achievements with their visibility status
SELECT code, name, tier, is_hidden,
       CASE WHEN is_hidden THEN 'HIDDEN' ELSE 'VISIBLE' END AS visibility
FROM achievements
ORDER BY is_hidden DESC, sort_order;
