...
export const TBSListItem = memo(function TBSListItem({
 ...
 <span className="text-sm text-gray-800 dark:text-[var(--foreground)] truncate">
 {title}
 </span>
 </div>
 ...
 <Link
 href={`/dashboard/simulations/${id}`}
 className="block bg-white dark:bg-[var(--card)] border border-gray-200 rounded-xl p-4 hover:border-[var(--primary)] hover:shadow-md transition-all group"
 >
 ...
 <h3 className="text-base font-semibold text-gray-800 dark:text-[var(--foreground)] group-hover:text-[var(--primary)] transition-colors">
 {title}
 </h3>
 <p className="text-sm text-gray-600 dark:text-[var(--muted)] mt-1 truncate">
 {topic}
 {subtopic && ` - ${subtopic}`}
 </p>
 </div>
