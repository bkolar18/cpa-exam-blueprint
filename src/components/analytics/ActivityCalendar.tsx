...
function getIntensityClass(minutes: number): string {
 if (minutes === 0) return 'bg-gray-100 dark:bg-[var(--card)]';
...
export default function ActivityCalendar({ data, year = new Date().getFullYear() }: ActivityCalendarProps) {
 ...
 <p className="text-xs text-gray-500 dark:text-[var(--muted)]">Active Days</p>
 </div>
 <div className="text-center">
 <p className="text-2xl font-bold text-green-600 dark:text-green-400">
 {Math.round(totalMinutes / 60)}h
 </p>
 <p className="text-xs text-gray-500 dark:text-[var(--muted)]">Total Study Time</p>
 </div>
 <div className="text-center">
 <p className="text-2xl font-bold text-green-600 dark:text-green-400">{currentStreak}</p>
 <p className="text-xs text-gray-500 dark:text-[var(--muted)]">Current Streak</p>
 </div>
 </div>
 ...
 {/* Legend */}
 <div className="flex items-center justify-end gap-1 mt-4 text-xs text-gray-500 dark:text-[var(--muted)]">
 <span>Less</span>
 <div className="w-3 h-3 rounded-sm bg-gray-100 dark:bg-[var(--card)]"/>
