...
export default function StateSelector({ states }: StateSelectorProps) {
 ...
 {/* Dropdown */}
 {isOpen && (
 <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[var(--card)] rounded-xl shadow-2xl border border-gray-200 overflow-hidden z-50">
 {/* Search input */}
 ...
 className="w-full px-4 py-2 bg-gray-50 dark:bg-[var(--card-hover)] border border-gray-200 dark:border-[var(--border)] rounded-lg text-[var(--foreground)] placeholder-gray-500 dark:placeholder-[var(--muted)] focus:outline-none focus:border-[var(--primary)] focus:ring-1 focus:ring-[var(--primary)]"
 autoFocus
 />
 </div>
 ...
 >
 <span className="font-medium text-[var(--foreground)] group-hover:text-white">
 {state.name}
 </span>
 <span className="text-sm text-gray-500 dark:text-[var(--muted)] group-hover:text-white/80">
 {state.code}
 </span>
 </button>
 ))
 ) : (
 <div className="px-4 py-3 text-gray-500 dark:text-[var(--muted)] text-center">
 No states found
 </div>
 )}
 </div>
