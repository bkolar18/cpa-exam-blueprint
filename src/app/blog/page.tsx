...
function PostCard({ post }: { post: ReturnType<typeof getAllPosts>[0] }) {
 ...
 <Link
 href={`/blog/${post.slug}`}
 className="group bg-white dark:bg-[var(--card)] rounded-xl border border-gray-200 overflow-hidden hover-lift"
 >
 <div className="p-6">
 ...
 <span className="text-sm text-gray-500 dark:text-[var(--muted)]">{post.readingTime} min read</span>
 </div>
 <h2 className="text-xl font-semibold text-[var(--foreground)] mb-2 group-hover:text-[var(--primary)] transition-colors">
 {post.title}
 </h2>
 <p className="text-gray-600 dark:text-[var(--muted-light)] mb-4 line-clamp-2">
 {post.description}
 </p>
 <div className="flex items-center justify-between text-sm">
 <span className="text-gray-500 dark:text-[var(--muted)]">{post.author}</span>
...
function FeaturedPost({ post }: { post: ReturnType<typeof getAllPosts>[0] }) {
 ...
 <Link
 href={`/blog/${post.slug}`}
 className="group block bg-gradient-to-br from-green-600 to-green-800 rounded-2xl overflow-hidden hover-lift"
 >
 <div className="p-8 md:p-10">
 <div className="flex items-center gap-3 mb-4">
 <span className="px-3 py-1 bg-white/20 rounded-full text-sm font-medium text-white flex items-center gap-1">
 <svg className="w-4 h-4"fill="currentColor"viewBox="0 0 20 20">
 <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
 </svg>
 Featured
 </span>
 <span className="text-sm text-green-100">{post.readingTime} min read</span>
 </div>
 <h2 className="text-2xl md:text-3xl font-bold text-white mb-3 group-hover:text-green-100 transition-colors">
 {post.title}
 </h2>
...
export default function BlogPage() {
 ...
 <div className="w-16 h-16 bg-gray-100 dark:bg-[var(--card)] rounded-full flex items-center justify-center mx-auto mb-4">
 ...
 <p className="text-gray-600 dark:text-[var(--muted-light)] max-w-md mx-auto mb-6">
 ...
 {/* CTA Section */}
 <section className="mt-16 bg-gray-100 dark:bg-[var(--card)] rounded-2xl p-8 md:p-12 text-center">
